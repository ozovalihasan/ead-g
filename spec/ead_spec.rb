...
describe EAD do
  let(:ead) { described_class.new }
  let(:file) { JSON.parse(File.read("#{__dir__}/ead_spec_sample.json")).to_json }
  ...
  describe '.import_JSON' do
    ...
    it 'imports JSON file with a custom path' do
      ...
      returned_file = ead.import_JSON(['custom.json'])
      expect(returned_file).to eq(file)
    end
  ...
  describe '.start' do
    ...
    it 'completes all necessary actions completely' do
      ...
      allow(ProjectFile).to receive(:add_line) do |name, end_model, line_content|
        changes << { file_name: name, type: 'model', end_model:, line: line_content, action: 'added' }
      end
      allow(ProjectFile).to receive(:update_line) do |name, type, keywords, line_content|
        changes << { file_name: name, type:, keywords:, line: line_content, action: 'updated' }
      end
      ...
      expect(changes).to contain_exactly(
        ...
        { file_name: 'add_assistant_professor_ref_to_user', type: 'reference_migration', keywords: /add_reference :users/, line: { 'null' => 'true', 'foreign_key' => '{ to_table: :teachers }' }, action: 'updated' },
